package com.gzq.learn.arithmetic.hash;

/**
 * ${DESCRIPTION}
 *
 * @author think
 * @created 2017-08-09 9:49.
 */
...
public class Shared<T> {
    ...
    private void init() {
        ...
        for (int i = 0; i < realNodes.size(); i++) {
            for (int j = 0; j < length; j++) {
                virtualNodes.put(hash1("aa" + i + j), realNodes.get(i));
            }
        }
    }
    ...
    public T getNode(String key) {
        Long hashedKey = hash1(key);
        // TODO judge null
        Map.Entry<Long, T> en = virtualNodes.ceilingEntry(hashedKey);
        if (en == null) {
